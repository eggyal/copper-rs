...
//!   wrap another [`EncodableType`]; one of either the [`defer`] or
//!   [`delegate`] macros should suffice for most implementations.
//!
...
///
/// Implementations of this trait are generated by the [`defer`], [`delegate`] and
/// [`CompoundType`] macros (as well as the crate-private `elements` macro), which are the
/// recommended ways to implement it.  Explicit implementations are likely only to be defined
/// locally in this crate, for upstream library types.
///
